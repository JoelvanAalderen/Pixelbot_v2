...
def run_bot_part(sim_id, bot_part, pixel_size, Placement, mass_values, stiffness_values, A_values, n_pixel_types, period, dt, t_final, start_p_position, start_p_angle, save_folder="solutions/climber_grid_search/flat_ground/0001_06max(natural_omega)/"):
    if not bot_part:
    ...
    for n, (bot_id, omega_sim, phi_sim) in enumerate(bot_part):
        ...
        shifted_points = [p+np.array([bot_offset_x, bot_offset_y]) for p in pts]
        shifted_vectors = [v.copy() for v in vecs]

        def ground_func_for_bot():
            return shifted_points, shifted_vectors

        bot_start_pos = (float(start_p_position[0] + bot_offset_x),
                         float(start_p_position[1] + bot_offset_y))
        ...
        for j in range(n_total_pixels):
            if j < n_actuated:
                if j == 0:
                    ...
                    phi_val = phi_sim + 1.2*np.pi*0
                else:
                    phi_val = phi_sim + 1.2*np.pi*1
        ...
        Ground_contact = Pixelbot_contact(bot, ground_func_for_bot, [offset_x, offset_y], name=f"contact_{bot_name}")
...
if __name__ == "__main__":
    ...
    phi_values = np.linspace(0, 0.8*np.pi, n_phi_options)
    ...
    filtered_results = [r for r in results if r["final_com_y"] < 0.03]
